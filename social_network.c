...
typedef struct //Data de nascimento do utilizador
  {
    int dia;
    int mes;
    int ano;
...
typedef struct Amigo
{
    ...
    int data; //Nº de amigos do utilizador
...
typedef struct pessoa //Perfil do utilizador
  {
    char Nome[Dim1];
    int idade;
    int friends; //Nº total de amigos
    float salario;
    DATA dt_nasc; //Data de nascimento
...
void Painel_Pesquisa()
{
  puts("\t\t\t\t*---------------------*---------------------*-----------------------*");
	puts("\t\t\t\t| 1.AREA DE INTERESSE | 2.LOCAL DE TRABALHO | 3.LOCAL DE NASCIMENTO |");
	puts("\t\t\t\t*---------------------*---------------------*-----------------------*");
}
...
{
    puts("\t\t*----------------------------*");
    puts("\t\t|**** M E N U  L O G I N ****|");
    puts("\t\t*----------------------------*");
    puts("\t\t   *----------------------*");
    puts("\t\t   | (A) ALTERAR PERFIL   |");
    puts("\t\t   *----------------------*");
    puts("\t\t   | (M) RETORNAR AO MENU |");
    puts("\t\t   *----------------------*");
}
...
void Remover_Utilizador(PESSOA vetor[],int i) //Remover utilizador
{
    ...
    vetor[i].area_interesse[0]= '\0';
}

void Log_In(PESSOA vetor[],int i) //LOG IN do utilizador
{
    char opt; //Opção do utilizador
    int num; //Opção do painel de edição
    Painel_LogIn(); //Invocação da função
    fflush(stdin); //Limpeza do buffer
    printf("Insira a opcao.\nR: "); scanf("%c",&opt); //Obtenção da resposta do utilizador
    switch(opt)
      {
        case 'a':
        case 'A':
          do
            {
              ...
              Alterar(vetor,i,num);
            } while(num != 0); //O ciclo permanece enquanto o inteiro for diferente de 0
          break; //Término do case
        case 'm':
        case 'M':
...
void Pesquisa(PESSOA vetor[],int num,int total)
{
    int i; //Contador do ciclo for
    int teste= 1;
    char stg[Dim2]; //String auxiliar
    switch(num)
      {
        case 1:
          fflush(stdin); //Limpeza do buffer
          printf("Insira a area de interesse pretendida.\nR: "); fgets(stg,Dim2,stdin);
          putchar('\n'); //Mudança de linha
          for(i=0; i<total; i++)
            {
              if(strcmp(vetor[i].area_interesse,stg)==0 && (vetor[i].Nome[0])!='\0') //As áreas de interesse coincidem
                {
                  Mostrar(vetor,i); //Impressão do perfil do utilizador
                  putchar('\n'); //Mudança de linha
                }
              else
                teste++;
          ...
          if(teste > total)
            printf("ERRO: NENHUM UTILIZADOR possui a area de interesse introduzida.\n");
          stg[0]= '\0'; //Limpeza da string
          break;
        case 2:
          fflush(stdin); //Limpeza do buffer
          printf("Insira o local de trabalho pretendido.\nR: "); fgets(stg,Dim2,stdin);
          putchar('\n'); //Mudança de linha
          for(i=0; i<total; i++)
            {
              if(strcmp(vetor[i].local_trb,stg)==0 && (vetor[i].Nome[0])!='\0')
                {
                  Mostrar(vetor,i); //Impressão do perfil do utilizador
                  putchar('\n'); //Mudança de linha
                }
          ...
          if(teste>total)
            printf("ERRO: NENHUM UTILIZADOR possui o local de trabalho introduzido.\n");
          stg[0]= '\0'; //Limpeza da string
          break;
        case 3:
          fflush(stdin); //Limpeza do buffer
          printf("Insira o local de nascimento pretendido.\nR: "); fgets(stg,Dim2,stdin);
          putchar('\n'); //Mudança de linha
          for(i=0; i<total; i++)
            {
              if(strcmp(vetor[i].local_nasc,stg)== 0 && (vetor[i].Nome[0])!='\0')
                {
                  Mostrar(vetor,i); //Impressão do perfil do utilizador
                  putchar('\n'); //Mudança de linha
                }
          ...
          if(teste>total)
            printf("ERRO: NENHUM UTILIZADOR possui o local de nascimento introduzido.\n");
          stg[0]= '\0'; //Limpeza da string
          break;
      }
    return;
...
int main(int argc,char **argv) //MENU PRINCIPAL
{
    ...
    int teste= 1;
    char resp; //Resposta do utilizador
    int searchoption; //Opção de pesquisa de utilizadores
        ...
        switch(opcao)
          {
              ...
              for(i=0; i<n; i++)
                {
                  if(strcmp(Perfil2,vetor[i].Nome)==0 && vetor[i].identificador== ID_aux)
                    ...
                    teste++; //Incrementação do inteiro
                }
              if(teste>n)
                printf("ERRO: Utilizador nao registado.\n");
              teste= 1;
              fflush(stdin); //Limpeza do buffer
            ...
            case 'p':
            case 'P':
              puts("Opcao PESQUISAR UTILIZADORES");
              putchar('\n'); //Mudança de linha
              Painel_Pesquisa();
              printf("Selecione a opcao para a pesquisa de utilizadores.\nR: "); scanf("%d",&searchoption);
              Pesquisa(vetor,searchoption,n); //Invocação da função
              fflush(stdin); //Limpeza do buffer
              printf("Prima \"R\" para retornar ao menu principal\nR: ");
              scanf("%c",&resp);
              switch(resp)
